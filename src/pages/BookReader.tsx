...
// Trỏ trực tiếp tới file trong public
...
export default function BookReader() {
  ...
  const [bookData, setBookData] = useState<BookData>({} as BookData)
  ...
  useEffect(() => {
    API.get(`/books/${params.id}/content/`)
    .then(async (res) => {
      ...
      if (bookInfo.pdf_url) {
        setIsLoading(true)
        try {
          const loadedPdf = await pdfjsLib.getDocument(bookInfo.pdf_url).promise
          setPdf(loadedPdf)
  ...
  const renderPage = (page: pdfjsLib.PDFPageProxy, scale: number) => {
  ...
  const handlePdfPageChange = async (direction: 'next' | 'prev') => {
    if (!pdf) return
    ...
    if (newPageNumber !== currentPage) {
      ...
      updateReadingProgress(newPageNumber)
      
      try {
        const page = await pdf.getPage(newPageNumber)
        setPdfPage(page)
      ...
      if (newPageNumber === pdf.numPages && !hasSubmittedReview) {
  ...
  const updateReadingProgress = (page: number) => {
    if (!pdf) return
    const progress = Math.round((page / pdf.numPages) * 100)
    setBookData(prev => ({
      ...prev,
      currentPage: page,
  ...
  const handleTextSelection = () => {
    // For PDF, we'll implement text selection differently
    // This is a simplified version - in a real implementation, you'd extract selected text from the PDF
  ...
  const saveNote = () => {
    if (selectedText && newNote) {
      if (editingNote) {
        ...
        setBookData(prev => ({
          ...prev,
          notes: prev.notes.map((n: BookNote) => 
            n.id === editingNote.id 
  ...
  const deleteNote = (noteId: string) => {
    setBookData(prev => ({
      ...prev,
      notes: prev.notes.filter((n: BookNote) => n.id !== noteId)
    }))
  }
  ...
  const toggleBookmark = () => {
    setBookData(prev => ({
      ...
      bookmarks: isBookmarked 
        ? prev.bookmarks.filter((page: number) => page !== currentPage)
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel className="text-xs uppercase tracking-wider">Zoom Level</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => { setScale(0.8); pdfPage && renderPage(pdfPage, 0.8) }} className="cursor-pointer">
                    <span className={scale === 0.8 ? 'font-bold text-primary' : 'font-medium'}>80%</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => { setScale(1.0); pdfPage && renderPage(pdfPage, 1.0) }} className="cursor-pointer">
                    <span className={scale === 1.0 ? 'font-bold text-primary' : 'font-medium'}>100%</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => { setScale(1.5); pdfPage && renderPage(pdfPage, 1.5) }} className="cursor-pointer">
                    <span className={scale === 1.5 ? 'font-bold text-primary' : 'font-medium'}>150%</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => { setScale(2.0); pdfPage && renderPage(pdfPage, 2.0) }} className="cursor-pointer">
                    <span className={scale === 2.0 ? 'font-bold text-primary' : 'font-medium'}>200%</span>
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="px-3 py-1.5 rounded-lg bg-muted/50 border border-border/30">
                ...
                <span className="text-xs font-medium text-muted-foreground">
                  {pdf ? pdf.numPages : bookData.totalPages || 0}
                </span>
              </div>
            ...
            <Card className="border-0 shadow-2xl bg-gradient-to-br from-card via-card to-card/95 backdrop-blur-sm overflow-hidden">
              <CardContent className="p-0">
                ...
                <div className="min-h-[calc(100vh-280px)] flex flex-col">
                  {/* PDF Content */}
                  <div 
                    className="flex-1 p-8 md:p-12 lg:p-16 flex items-center justify-center"
                    style={{ 
                      maxWidth: '100%', 
                      marginLeft: 'auto', 
                      marginRight: 'auto',
                  ...
                  >
                    {isLoading ? (
                      ...
                      <div className="flex flex-col items-center">
                        <canvas 
                          ref={canvasRef} 
                          className="max-w-full h-auto shadow-2xl rounded-lg border border-border/50"
                          onDoubleClick={handleTextSelection}
                        />
                        <div className="mt-4 text-sm text-muted-foreground">
                          Double-click on text to add notes
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="px-8 md:px-12 lg:px-16 pb-8 pt-6 border-t border-border/30 bg-gradient-to-b from-transparent to-muted/20">
                    <div className="flex items-center justify-between gap-4" style={{ maxWidth: '100%', marginLeft: 'auto', marginRight: 'auto', width: '100%' }}>
                      <Button 
                        variant="ghost"
                        size="lg"
          ...
          <div className="xl:col-span-4 space-y-5">
            ...
            <Card className="border-0 shadow-xl bg-gradient-to-br from-card via-card to-card/95 backdrop-blur-sm hover:shadow-2xl transition-shadow duration-300">
              ...
              <CardContent className="pt-5 space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar">
                {bookData.notes && bookData.notes.map((note: BookNote) => {
                  const colorStyles = {
                    blue: {
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => {
                        ...
                        // In a real implementation, you would also navigate to the PDF page
                      }}
                    >
                      {/* Highlighted Text */}
                ...
                {(!bookData.notes || bookData.notes.length === 0) && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-xs text-muted-foreground mt-2 max-w-[200px] mx-auto">
                      Double-click on PDF text to create your first note
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            <Card className="border-0 shadow-xl bg-gradient-to-br from-card via-card to-card/95 backdrop-blur-sm hover:shadow-2xl transition-shadow duration-300">
              ...
              <CardContent className="pt-5">
                <div className="space-y-2">
                  {bookData.bookmarks && bookData.bookmarks.map((page: number) => (
                    <Button
                      key={page}
                      variant="ghost"
                      size="sm"
        ...
        <DialogContent className="sm:max-w-[580px] max-h-[90vh] p-0 gap-0 overflow-hidden flex flex-col">
          ...
          <div className="p-4 space-y-4 overflow-y-auto flex-1">
            ...
            <div className="rounded-xl bg-muted/50 p-3 border border-border/50">
              ...
              <p className="text-xs text-muted-foreground mb-2">by {bookData.author || "Unknown Author"}</p>
              <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>{pdf ? pdf.numPages : bookData.totalPages || 0} pages</span>
                </div>
                <div className="flex items-center gap-1">
                  <StickyNote className="h-3 w-3" />
                  <span>{bookData.notes?.length || 0} notes</span>
                </div>
              </div>
            </div>
          </div>
