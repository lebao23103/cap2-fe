...
// Tech stack data - with brand colors for hover effects
const techStack = [
  ...
  return (
    <SmoothScroll>
      <div ref={containerRef} className='relative min-h-screen bg-background overflow-hidden font-mono'>
        ...
        {/* Improved Tech Stack - Infinite Marquee */}
          ...
          <div className="w-full relative">
            <div className="flex w-full overflow-hidden mask-gradient">
              <motion.div
                ...
                whileHover={{ animationPlayState: "paused" }} // Note: This doesn't work directly with framer motion animate prop. We need to use state control for true pause or CSS.
              // However, slowing it down to 40s makes it readable even without pause. 
              // A true pause with Framer Motion usually requires useAnimation controls or distinct handling.
              // For simplicity and performance, we'll rely on the slower speed + hover interaction on individual cards.
              >
                {/* Quadruple the stack for smoother loop */}
                {[...techStack, ...techStack, ...techStack, ...techStack].map((tech, index) => (
                  <div
                    key={index}
                    className="flex-shrink-0 w-48 group cursor-default relative"
                  >
                    <div
                      className={`
                        flex flex-col items-center gap-4 px-6 py-6 
                        bg-white dark:bg-black border-2 border-border 
                      ...
                      `}
                      style={{
                        // We can use CSS variables for hover effects if we want dynamic coloring
                        // But for "Visual Balance", let's keep it monochrome until hover
                      }}
                    >
                      {/* Hover Glow Effect using the brand color */}
                      <div
                        className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-10 transition-opacity duration-300"
                        style={{ backgroundColor: tech.color }}
                      />

                      <div className="h-16 w-16 bg-transparent rounded-lg flex items-center justify-center p-2 transform group-hover:scale-110 transition-transform duration-300">
                        <img
                          src={tech.logo}
                          alt={tech.name}
                      ...
                      <span className="font-bold text-sm uppercase tracking-wider text-black dark:text-white text-center group-hover:text-primary transition-colors duration-300">{tech.name}</span>
                    </div>

                    {/* Background accent that appears on hover */}
                    <div
                      className="absolute inset-0 bg-black dark:bg-white rounded-xl translate-x-1 translate-y-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 -z-10"
                    />
                  </div>
                ))}
              </motion.div>
            ...
            {/* Gradients to mask edges - Adjusted for both modes */}
            <div className="absolute top-0 left-0 w-32 h-full bg-gradient-to-r from-white dark:from-zinc-900 to-transparent z-10 pointer-events-none" />
            <div className="absolute top-0 right-0 w-32 h-full bg-gradient-to-l from-white dark:from-zinc-900 to-transparent z-10 pointer-events-none" />
          </div>
        </section>
