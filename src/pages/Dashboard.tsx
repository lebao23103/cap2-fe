...
import { BookNotesGroup } from '@/components/dashboard/BookNotesGroup'
                    ...
                    {userNotes.length > 0 ? (
                      <Accordion type="multiple" className="space-y-4">
                        {Object.entries(userNotes.reduce((acc: any, note: any) => {
                          ...
                          // Find book ID from first note to lookup cover
                          const bookId = notes[0]?.book;
                          // Lookup book in readingHistory or use defaults
                          const bookData = readingHistory.find((b: any) => b.id == bookId || b.title === bookTitle);
                          const coverImage = bookData?.cover || undefined;
