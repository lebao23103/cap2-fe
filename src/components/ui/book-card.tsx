import { Card, CardContent } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { Star } from 'lucide-react'
import { cn } from '@/lib/utils'
import { Link } from 'react-router-dom'

export interface BookData {
  id: string
  title: string
  author: string
  cover: string
  rating: number
  genre: string[]
}

export interface BookCardProps {
  book: BookData
  size?: 'sm' | 'md' | 'lg'
  className?: string
  onClick?: () => void
}

const sizeVariants = {
  sm: {
    container: 'p-3 flex gap-3 items-start',
    cover: 'w-12 h-16',
    title: 'text-sm text-left',
    author: 'text-xs text-left',
    rating: 'w-3 h-3',
    badge: 'text-[10px] px-2 py-0.5',
    cardShadow: 'shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]',
    hoverShadow: 'hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]',
    imageOffset: 'translate-y-0.5 translate-x-0.5',
    layout: 'horizontal'
  },
  md: {
    container: 'p-4 flex flex-col items-start gap-3 h-full',
    cover: 'w-full aspect-[2/3]',
    title: 'text-base',
    author: 'text-sm',
    rating: 'w-3.5 h-3.5',
    badge: 'text-xs px-2.5 py-0.5',
    cardShadow: 'shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]',
    hoverShadow: 'hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]',
    imageOffset: 'translate-y-1 translate-x-1',
    layout: 'vertical'
  },
  lg: {
    container: 'p-6 flex flex-col items-start gap-4 h-full',
    cover: 'w-full aspect-[2/3]',
    title: 'text-lg',
    author: 'text-base',
    rating: 'w-4 h-4',
    badge: 'text-sm px-3 py-1',
    cardShadow: 'shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]',
    hoverShadow: 'hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]',
    imageOffset: 'translate-y-1.5 translate-x-1.5',
    layout: 'vertical'
  }
}

export default function BookCard({ book, size = 'md', className, onClick }: BookCardProps) {
  const variant = sizeVariants[size]
  const isVertical = variant.layout === 'vertical'

  const Content = (
    <Card
      className={cn(
        "group transition-all duration-300 cursor-pointer overflow-hidden border-2 border-border",
        "bg-card hover:bg-card",
        "hover:-translate-y-1 hover:-translate-x-1",
        variant.cardShadow,
        variant.hoverShadow,
        "active:translate-y-0 active:translate-x-0 active:shadow-none",
        "rounded-xl h-full",
        className
      )}
      onClick={onClick}
    >
      <CardContent className={cn(variant.container, "relative z-10")}>
        <div className={cn("flex-shrink-0 relative group-hover:scale-105 transition-transform duration-500", isVertical ? "w-full" : "")}>
          <div className={cn("absolute inset-0 bg-black rounded-md", variant.imageOffset)} />
          <img
            src={book.cover}
            alt={book.title}
            loading="lazy"
            className={cn(
              variant.cover,
              "object-cover border-2 border-border relative z-10 bg-card rounded-md w-full"
            )}
          />
        </div>

        <div className="flex-1 min-w-0 w-full flex flex-col">
          <h3 className={cn(
            "font-bold line-clamp-2 mb-1 text-foreground group-hover:underline decoration-2 underline-offset-2 uppercase",
            variant.title
          )}>
            {book.title}
          </h3>
          <p className={cn(
            "text-gray-600 line-clamp-1 font-mono uppercase",
            variant.author
          )}>
            {book.author}
          </p>

          <div className={cn("flex flex-wrap gap-2 mt-auto pt-3", isVertical ? "justify-between w-full items-center" : "flex-col items-start")}>
            <div className="flex items-center gap-1">
              <Star className={cn(
                "fill-black text-black dark:fill-white dark:text-white",
                variant.rating
              )} />
              <span className="text-xs font-bold text-foreground font-mono">
                {book.rating.toFixed(1)}
              </span>
            </div>

            <div className="flex gap-1.5 flex-wrap">
              {book.genre.slice(0, 1).map((g) => (
                <Badge
                  key={g}
                  variant="secondary"
                  className={cn(
                    "bg-card text-foreground border-2 border-border rounded-full shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] dark:shadow-[1px_1px_0px_0px_rgba(255,255,255,1)] hover:translate-y-[-1px] hover:translate-x-[-1px] transition-transform",
                    variant.badge
                  )}
                >
                  {g}
                </Badge>
              ))}
            </div>
          </div>
        </div>
      </CardContent>
    </Card>
  )

  if (onClick) {
    return Content
  }

  return (
    <Link to={`/book/${book.id}`} className="block h-full">
      {Content}
    </Link>
  )
}
